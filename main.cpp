...
int main() {
    // input sequence
//    double inputs[SEQ_LENGTH]{0.0517, 0.3158, -0.432, 0.2567};
...
//    double w_array[NUM_UNITS * 4]{0.3581, 0.1449, -0.1517, -0.0627, 0.2185, 0.3742, 0.1810, -0.3759, 0.5075, -0.0862, 0.4901, 0.4955};
    ...
    // bias array
//    double w_bias[NUM_UNITS * 4]{-0.3353, -0.3428, -0.5422, -0.1341, -0.0048, 0.2355, 0.2891, -0.3969, 0.2711, 0.1751, -0.0789, -0.3125};
    ...
    double u_array[NUM_UNITS][NUM_UNITS * 4] = {{-0.0916, -0.2958, -0.3029,  0.4635, -0.3798, -0.6660,  0.3647, -0.3648,
                                                        -0.2473,  0.1977,  0.1243,  0.1642,  0.5087,  0.2351,  0.3023, -0.6224,
                                                        -0.4155,  0.0496,  0.1967,  0.4653},
                                                {0.2413, -0.3172, -0.3208, -0.0284,  0.5316, -0.3189, -0.2269, -0.0008,
                                                        0.1918, -0.1530,  0.3306, -0.1718, -0.2074, -0.1121, -0.5950, -0.4636,
                                                        0.1989, -0.0902, -0.4454,  0.6904},
                                                {-0.2769, -0.3546,  0.4945,  0.2046,  0.0170,  0.4559,  0.4595,  0.1425,
                                                        0.3532, -0.1646, -0.6491, -0.2808, -0.1576, -0.0327,  0.1833, -0.2814,
                                                        0.2739, -0.0235, -0.1867,  0.0802},
                                                {-0.0501,  0.1581, -0.1047,  0.2665, -0.1189, -0.0026,  0.1278,  0.0753,
                                                        -0.0041, -0.1752,  0.0652,  0.1395,  0.0660,  0.5631,  0.1124,  0.5302,
                                                        0.0814,  0.5548,  0.1739, -0.4648},
                                                {-0.5395, -0.0421, -0.1753,  0.0589,  0.3069, -0.1285, -0.3267, -0.1625,
                                                        -0.1848,  0.0712,  0.0601,  0.3863, -0.7180, -0.4330, -0.4514, -0.0106,
                                                        -0.0300, -0.6793, -0.6978,  0.1530}};

    // hh bias
    double u_bias[NUM_UNITS * 4]{-0.3236,  0.4668,  0.6738,  0.5599,  0.1805, -0.3578, -0.0777, -0.0739,
                                 0.0458, -0.3726, -0.1154,  0.0794,  0.1378,  0.0076,  0.0043, -0.2654,
    ...
    for (int j = 0; j < SEQ_LENGTH; j++) {
        ...
        for (int i = 0; i < NUM_UNITS; i++) {
            ...
            for (int k = 0; k < NUM_UNITS; k++) {
                adjusted_inputs[i * 4] += units[k].hidden_state_ * u_array[k][i];
                adjusted_inputs[i * 4 + 1] += units[k].hidden_state_ * u_array[k][NUM_UNITS * 1 + i];
                adjusted_inputs[i * 4 + 2] += units[k].hidden_state_ * u_array[k][NUM_UNITS * 2 + i];
                adjusted_inputs[i * 4 + 3] += units[k].hidden_state_ * u_array[k][NUM_UNITS * 3 + i];
            }
        ...
        // Unit Operations
        for (int i = 0; i < NUM_UNITS; i++) {
            units[i].performInference(adjusted_inputs[i * 4],
                                      adjusted_inputs[i * 4 + 1],
                                      adjusted_inputs[i * 4 + 2],
                                      adjusted_inputs[i * 4 + 3]);
        }
    }
    ...
    for (int i = 0; i < NUM_UNITS; i++) {
        dense_result += dense_weights[i] * units[i].hidden_state_;
    }
    dense_result += dense_bias;
    std::cout << "Result: " << dense_result << '\n';

    // Inverse Transform
    std::cout << "Inverse Transformed: " << ((dense_result+3.40463247)/0.02363507) <<"\n";
